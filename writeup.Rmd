...
In order to capture such seasonality and periodicity, we may consider a SEIR model with a seasonal contact rate $\beta$, since the Susceptible is rather constant in such a short time(2 years), and around 1971 there was a big vaccine [push](#ref)$^{[4][5]}$.
...
We choose starting value to be
...
In terms of parameters, $b_{1}$, $b_{2}$, $\phi$, $\rho$ all have a clear trend of convergence, but $\eta$ moves with high variability.
...
The likelihood approximation generated by the final filtering iteration may not be that reliable for multiple reasons, so same as what we did in the class, we use replicated particle filters to evaluate both likelihood and standard error again at each point estimate.
...
The local maximization of likelihood function yields several combination of parameters, as we tabulated them in the above table. Now we select the parameters that corresponds to the largest likelihood to fit our model, and simulate the data to see how well the fit is going to be.
...
The plot suggests that our current model is much better than the initial one, as it can describe the seasonal characteristics that simple SIR model can not.
...
```{r global_fig, message=FALSE, warning=FALSE, fig.cap=cap_fig9}
cap_fig9 = paste( 
  "**Figure 9.** *Values of parameters corresponding to each global iteration*",
  "Starting value is randomly selected."
)
...
The shape of log likelihood is somehow like a cliff, and it can be explained by the bad choices of starting parameters. Note that, this kind of behavior actually is likely to happen when the dimension of parameter space increases. The sparsity nature means that a thorough search is computationally expensive, since basically our algorithm is using **Uniform distribution** to sample parameters from the parameter space. So your starting point is very likely to be away from the optimal point, this is why some likelihood is so low at the first few iterations and then suddenly increase dramatically. The [plot](#ref)$^{[7]}$ below illustrates the sparsity of neighborhood of one point in a high dimensional space.

```{r sparsity, fig.align = 'center', fig.cap = cap_fig8, out.width="80%"}
cap_fig8 = paste(
  "**Figure 8.** *The curse of dimensionality is well illustrated by a subcubical
...
```{r global_fig_pair, fig.cap=cap_fig10, message=FALSE, warning=FALSE}
cap_fig10 = paste( 
  "**Figure 10.** *Pairwaise relations between parameters*",
  "Global search."
)
pairs( ~ logLik + b1 + b2 + Phi+ rho + eta ,
...
Think back to the question we considered at the beginning of the report, now we can say that mumps cases of Michigan in the 1970s can be well modeled by an SEIR pomp model, and the key to do this is to add a seasonal contact rate. Parameters selection can be time-consuming, the fit may be not that perfect due to randomness of simulation, but what really matters is that the seasonality have already been captured by this model, and usually our model only differs by a constant from the orginal data.

## Limitation

- The curse of dimension makes it computationally expensive to maximize the likelihood function globally.
- The problem of depletion of susceptible still exists, and our model may not work if the time series is sampled over a long time span, like 10+ years.
