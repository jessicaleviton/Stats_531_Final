...
params <- c(Beta=60, rho=0.01, eta=0.416, mumps_fixed_params)
...
mumps_box <- rbind(
  Beta=c(5,80), eta=c(0,0.60), rho=c(0, 0.9)
)

m_global <- foreach(i=1:mumps_Nreps_global, .packages='pomp', 
  .combine=c) %dopar% mif2(m_local[[1]],
...
lik_global <- foreach(i=1:mumps_Nreps_global, .packages='pomp',
  .combine=rbind) %dopar% logmeanexp(
    replicate(mumps_Nreps_eval,
      logLik(pfilter(mumpSIR,
        params=coef(m_global[[i]]), Np=mumps_Np))),
    se=TRUE)
