...
mumps_MI_70 <- subset(mumps_MI, (time > 1970) & (time < 1973), select=c("time", "reports"))

plot(mumps_MI_70$time)

mumps_MI_70_t0 <- 1970+1/52
tail(mumps_MI)
head(mumps_MI_70)
#length(mumps_MI_70$week)


plot(mumps_MI_70$reports~mumps_MI_70$time, type='l')
```
...
params <- c(Beta=20, mu_IR=1, rho=0.5, eta=0.1, N=9e6)
mumps_fixed_params <- c(N=9e6, mu_IR=1)
...
logLik(pfilter(mumpSIR, params=coef(m2[[1]]), Np=mumps_Np))
...
```{r global}
mumps_box <- rbind(
  Beta=c(5, 5e4), eta=c(0, 0.4), rho=c(0.2, 0.9)
)
