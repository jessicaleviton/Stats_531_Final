...
mumps_data %>%
  pomp(
    times="week", t0=0,
    rprocess=euler(seir_step, delta.t=1/21),
    rinit=seir_init,
    rmeasure=rmeas,
    dmeasure=dmeas,
    accumvars="H",
...
params <- c(Beta=60, rho=0.8, eta=0.0216, b1=3, mumps_fixed_params)
...
mumps_box <- rbind(
  Beta=c(5,80), eta=c(0,0.10), rho=c(0, 0.9)
)
...
best_params <- c(Beta=fit_params$Beta, rho=fit_params$rho, eta=fit_params$eta, mumps_fixed_params)
